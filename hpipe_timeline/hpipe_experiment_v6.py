...
os.environ['MASTER_PORT'] = '29660' # 新端口
os.environ["NCCL_P2P_DISABLE"] = "1"
os.environ["NCCL_IB_DISABLE"] = "1"
os.environ["NCCL_SHM_DISABLE"] = "1"
os.environ["KMP_DUPLICATE_LIB_OK"] = "TRUE"

OUTPUT_DIR = "hpipe_exp_results_v6"
if not os.path.exists(OUTPUT_DIR): os.makedirs(OUTPUT_DIR)

# [修正1] 内置经验基准值 (秒)，防止 Warmup 探测失效
# 这些值是根据之前的 CSV 数据估算的 Rank 1 纯计算耗时
MODEL_BASE_LATENCY = {
    'resnet50': 0.020,     # 20ms
    'mobilenet_v2': 0.015, # 15ms
    'vit': 0.025,          # 25ms
    'llama': 0.005         # 5ms
}
...
def run_worker_scenario(rank, world_size, model_name, plan_a, plan_b, all_nodes, node_shapes, mode, results_dict):
    try:
        ...
        # [修正2] 设定 5.0 倍变慢因子 (Drop to ~20% throughput)
        SLOWDOWN_FACTOR = 5.0 
        # 获取该模型的基准延迟
...
def run_experiment_for_model(model_name):
    print(f"\n=== Running Experiment v6: {model_name} ===")
    ...
    if 'baseline_latency' in results:
        ...
        # [修正4] Summary 统计逻辑: 仅统计恢复期 (Batch 75-100)
        # 此时系统已经稳定在“带病运行”状态，Hpipe 应该明显快于 Baseline
        eval_window = list(range(75, 100))
